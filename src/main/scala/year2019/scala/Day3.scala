...
object Day3 extends App {
  def getPaths(seq: Seq[String]) = {
    seq.scanLeft((Set[History](), History(0, 0)))((pathsAndHistory, path) => {
      val history = Day3Util.createPathsForHistory(pathsAndHistory._2, path)
  ...
  def pathsIntersections(set1: Seq[String], set2: Seq[String]) = {
    val paths1 = getPaths(set1)
    val paths2 = getPaths(set2)
    paths1.intersect(paths2)
      .filter(h => h.y != 0 && h.x != 0)

  }

  def part1(): Int = {
    val seq = Content.input.split("\n")
    val set1 = seq(0).split(",")
    val set2 = seq(1).split(",")
    pathsIntersections(set1, set2)
      .map(h => Math.abs(h.x) + Math.abs(h.y))
      .min
  }

  println(pathsIntersections(Seq("R98", "U47", "R26", "D63", "R33", "U87", "L62", "D20", "R33", "U53", "R51"), Seq("U98", "R91", "D20", "R16", "D67", "R40", "U7", "R15", "U6", "R7")).map(h => Math.abs(h.x) + Math.abs(h.y)).min)
  println(part1())
